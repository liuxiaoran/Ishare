...
public class HttpDataFetcher {
    ...
    protected HttpDataResult doRequest(HttpRequestExtInterface request) {
        ...
        try {
            if (request.isGzip() && mUseGzip) {
                request.getRequestBase().setHeader("Accept-Encoding", "gzip,deflate");
            }
            request.getRequestBase().addHeader("User-Agent", AppConst.HTTP_AGENG);

            httpClient = HttpConnManager.getHttpClient();

            long startTime = System.currentTimeMillis();

            Log.v(TAG, "Uri-->" + request.getRequestBase().getURI());

            HttpResponse httpResponse = httpClient.execute(request.getRequestBase());
            int responseCode = httpResponse.getStatusLine().getStatusCode();

            Log.v(TAG, "Uri-->" + request.getRequestBase().getURI() + "..." + responseCode + "..." + (System.currentTimeMillis() - startTime));
...
//            int dataLength = (int)httpResponse.getEntity().getContentLength();
            ...
            Log.e(TAG, e.toString() + request.getRequestBase().getURI(), e);
    ...
    public HttpDataResult execute(HttpRequestExtInterface request) {
